...
function reset(){form.reset();id.value="";sort.value="0";active.checked=true;dismissible.checked=true;showOnce.checked=false;ctaText.value="";ctaUrl.value="";preview.hidden=true;preview.innerHTML="";save.textContent="Save announcement";image.value="";const removeImage=document.getElementById("announcement-remove-image");if(removeImage){removeImage.checked=false;removeImage.disabled=true;removeImage.parentElement.hidden=true;}}
...
function edit(recordId){const a=records.find(x=>x.id===recordId);if(!a)return;id.value=a.id;title.value=a.title;description.value=a.short_description;target.value=a.target_page;mode.value=a.display_type||((a.display_mode==="banner")?"banner":"inline");sort.value=a.sort_order;ctaText.value=a.cta_text||"";ctaUrl.value=a.cta_url||"";dismissible.checked=a.dismissible!==false;showOnce.checked=Boolean(a.show_once);start.value=toLocal(a.starts_at);expires.value=toLocal(a.expires_at);active.checked=a.is_active;image.value="";const removeImage=document.getElementById("announcement-remove-image");if(a.image_path){preview.hidden=false;preview.innerHTML=`<img src="${escapeHtml(imageUrl(a.image_path))}" alt="Announcement image preview">`;if(removeImage){removeImage.checked=false;removeImage.disabled=false;removeImage.parentElement.hidden=false;}}else{preview.hidden=true;preview.innerHTML="";if(removeImage){removeImage.checked=false;removeImage.disabled=true;removeImage.parentElement.hidden=true;}}save.textContent="Update announcement";window.scrollTo({top:0,behavior:"smooth"});}
