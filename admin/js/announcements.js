...
function reset(){form.reset();id.value="";sort.value="0";active.checked=true;preview.hidden=true;preview.innerHTML="";save.textContent="Save announcement";image.value="";}
...
function render(){rows.innerHTML=records.map(a=>`<tr><td><div class="announcement-row-title">${a.image_path?`<div><img class="announcement-thumb" src="${escapeHtml(imageUrl(a.image_path))}" alt="">`: "<div>"}<strong>${escapeHtml(a.title)}</strong></div><small>${escapeHtml(a.short_description)}</small></div></td><td>${escapeHtml(a.target_page)}</td><td>${escapeHtml(a.display_mode)}</td><td>${a.sort_order}</td><td>${a.is_active?'<span class="badge">Active</span>':'Inactive'}</td><td><button class="btn btn-secondary" data-edit="${a.id}">Edit</button> <button class="btn btn-secondary" data-toggle="${a.id}">${a.is_active?"Unpublish":"Publish"}</button></td></tr>`).join("")||'<tr><td colspan="6" class="muted">No announcements yet.</td></tr>';
...
function edit(recordId){const a=records.find(x=>x.id===recordId);if(!a)return;id.value=a.id;title.value=a.title;description.value=a.short_description;target.value=a.target_page;mode.value=a.display_mode;sort.value=a.sort_order;start.value=toLocal(a.starts_at);expires.value=toLocal(a.expires_at);active.checked=a.is_active;image.value="";if(a.image_path){preview.hidden=false;preview.innerHTML=`<img src="${escapeHtml(imageUrl(a.image_path))}" alt="Announcement image preview">`;}else{preview.hidden=true;preview.innerHTML="";}save.textContent="Update announcement";window.scrollTo({top:0,behavior:"smooth"});}
...
form.addEventListener("submit",async e=>{e.preventDefault();clearStatus();save.disabled=true;save.textContent="Saving…";let uploaded=null;try{const existing=records.find(x=>x.id===id.value);const file=image.files?.[0];if(file)uploaded=await upload(file);const payload={title:title.value.trim(),short_description:description.value.trim(),target_page:target.value,display_mode:mode.value,sort_order:Math.max(0,Number.parseInt(sort.value,10)||0),starts_at:start.value?new Date(start.value).toISOString():null,expires_at:expires.value?new Date(expires.value).toISOString():null,is_active:active.checked,image_path:uploaded||existing?.image_path||null};let error;if(id.value){({error}=await supabase.from("announcements").update(payload).eq("id",id.value));if(error)throw error;if(uploaded&&existing?.image_path)await remove(existing.image_path);}else{({error}=await supabase.from("announcements").insert(payload));if(error)throw error;}show("Announcement saved.");reset();await load();}catch(e){if(uploaded)await remove(uploaded);show(e?.message||"Could not save announcement.",true);}finally{save.disabled=false;if(!id.value)save.textContent="Save announcement";}});
image.addEventListener("change",()=>{const file=image.files?.[0];if(!file){preview.hidden=true;return;}try{validateFile(file);preview.hidden=false;preview.innerHTML=`<img src="${escapeHtml(URL.createObjectURL(file))}" alt="Announcement image preview">`;}catch(e){image.value="";show(e.message,true);}});
