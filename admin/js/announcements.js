...
image.addEventListener("change",()=>{const file=image.files?.[0];if(!file){syncRemoveImageControl();return;}try{validateFile(file);removeCurrentImage=false;renderImagePreview(URL.createObjectURL(file),"Selected announcement image",false);syncRemoveImageControl();}catch(e){image.value="";show(e.message,true);}});

function renderImagePreview(src,alt,isExisting){preview.hidden=false;preview.innerHTML=`<img src="${escapeHtml(src)}" alt="${escapeHtml(alt)}"><button class="announcement-image-remove-x" type="button" aria-label="${isExisting?"Remove current image":"Remove selected image"}" title="${isExisting?"Remove current image":"Remove selected image"}">×</button><button class="announcement-image-undo" type="button" hidden>Undo removal</button>`;const removeButton=preview.querySelector(".announcement-image-remove-x");const undoButton=preview.querySelector(".announcement-image-undo");removeButton.addEventListener("click",()=>{if(isExisting){removeCurrentImage=true;preview.classList.add("is-marked-for-removal");removeButton.hidden=true;undoButton.hidden=false;show("Current image will be deleted when you save this announcement.");}else{image.value="";preview.hidden=true;preview.innerHTML="";}});undoButton.addEventListener("click",()=>{removeCurrentImage=false;preview.classList.remove("is-marked-for-removal");removeButton.hidden=false;undoButton.hidden=true;clearStatus();});}

function syncRemoveImageControl(){const hasExisting=Boolean(id.value && records.find(x=>x.id===id.value)?.image_path);if(!hasExisting){removeCurrentImage=false;}const removeButton=preview.querySelector(".announcement-image-remove-x");const undoButton=preview.querySelector(".announcement-image-undo");if(removeButton){removeButton.hidden=removeCurrentImage;removeButton.disabled=removeCurrentImage;}if(undoButton){undoButton.hidden=!removeCurrentImage;}preview.classList.toggle("is-marked-for-removal",removeCurrentImage);}
